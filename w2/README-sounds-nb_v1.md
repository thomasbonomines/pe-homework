...
# votre code
def sine(freq, duration=1, amplitude=1.):
    tab = np.arange(RATE*duration)
    return np.sin(2 * np.pi * freq * (tab / RATE))
```
...
def sine_linear(freq1, freq2, duration):
    tab = np.arange(RATE*duration)
    tab = np.sin(2 * np.pi * (freq1 + tab *(freq2 - freq1)/(duration*RATE)) * (tab / RATE))
    x = np.arange(RATE*duration)
    return tab
```
...
```{code-cell} ipython3
# votre code pour 1.
crescendo_la_1seconde = la_1seconde * x
```
...
def crescendo_sine(freq, duration):
    tab = np.arange(RATE * duration)
    num = np.arange(RATE * duration)
    tab = np.sin(2 * np.pi * freq * (tab / RATE))
    return tab * num
```
...
```{code-cell} ipython3
# votre code pour 3.
def crescendo_sine3(freq, duration, increase=True):
    if increase:
        return crescendo_sine(freq,duration)
    else:
        return sine(freq, duration, 1.)
```
...
def float_to_int16(as_float):
    x = max(np.abs(max(as_float)), np.abs(min(as_float)))
    as_float = as_float * (2**15 / x) 
    return as_float.astype(np.int16)
```
...
ratios = [(2**(1/12))**i for i in range(13)]
```
...
def freq_from_name(name):
    base = 440 / ratios[9]
    scale1 = np.array(scale)
    indice = np.where(scale1 == name)[0][0]
    return np.floor(base * ratios[indice])
...
# à vous
"On fait des inégalités, on teste si x>=y et si y>=x"
"Ou sinon, np.isclose(x,y,2**(-4))"
```
...
wavfile.write("sample.wav", RATE, original)
```
...
restored = wavfile.read("sample.wav")[1] # relisez le fichier 'sample.wav' dans une variable 'restored'
print(restored)
```
...
```{code-cell} ipython3
data = wavfile.read("media/sounds-cello.wav")[1]
```
...
```{code-cell} ipython3
rate1 = wavfile.read("media/sounds-cello.wav")[0]
```
...
add = np.zeros(RATE*delay)
data_echoed = np.concatenate((add,data))
plt.plot(data_echoed)
plt.show()
data2 = np.concatenate((data,add))
plt.plot(data2)
plt.show()
final = data_echoed*0.3 + data2*0.7
plt.plot(final[:len(data)])
plt.show()
final = final[:len(data)]
```
...
# votre code
add = np.zeros(RATE*delay)
data_echoed_v2 = np.concatenate((add,data))
plt.plot(data_echoed)
plt.show()
data2 = np.concatenate((data,add))
final2 = data_echoed_v2*0.3 + data2*0.7
plt.plot(final)
plt.show()
MyAudio(final2)
```
